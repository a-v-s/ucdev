...
void ccs811_init(ccs811_t *css811) {
...
int css811_measure(ccs811_t *css811) {

	// Not getting sensible results yet

	ccs811_alg_result_data_t alg_result_data;

	bshal_i2cm_recv_reg(css811->p_i2c, css811->addr, CCS811_REG_ALG_RESULT_DATA,
			&alg_result_data, 8);
	uint8_t error = 0;
	if (alg_result_data.status.error) {
		bshal_i2cm_recv_reg(css811->p_i2c, css811->addr,
		CCS811_REG_ERROR_ID, &error, 1);
	}

	if (alg_result_data.status.data_ready) {
		// Incoming data is big endian
		// 400ppm to 8192ppm
		// 0ppb to 1187ppb

		uint16_t eCO2 = be16toh(alg_result_data.eCO2);
		uint16_t TVOC = be16toh(alg_result_data.TVOC);
